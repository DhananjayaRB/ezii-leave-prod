...
const fetchEmployeesFromAPI = async (
  ...
  try {
    ...
    console.log(
      "[ExternalAPI] Full request payload:",
      JSON.stringify(payload, null, 2),
    );

    // Call the new employees API endpoint
    const response = await fetch(
      "https://apiv1.resolvepay.in/organization/employees?page=1&pageSize=0",
      {
        method: "POST",
        headers: {
    ...
    if (
      ...
      !apiData.data ||
      !Array.isArray(apiData.data.employee_data)
    ) {
    ...
    const transformedData: Employee[] = apiData.data.employee_data.map(
      (emp: any) => ({
    ...
    console.log(
      "[ExternalAPI] Transformed employee data from new API:",
      transformedData,
    );
    return transformedData;
  } catch (error) {
    console.error(
      "[ExternalAPI] Error fetching employees from new API:",
      error,
    );
    throw error;
  }
...
const fetchWorkerTypes = async (): Promise<WorkerType[]> => {
  try {
    ...
    const response = await fetch(
      "https://apiv1.resolvepay.in/organization/customer/worker-type-list",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
...
const fetchAttributes = async (): Promise<Attribute[]> => {
  try {
    ...
    const response = await fetch(
      "https://apiv1.resolvepay.in/organization/attribute-list",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
...
const fetchSubAttributes = async (
  ...
  try {
    ...
    const response = await fetch(
      `https://apiv1.resolvepay.in/organization/attribute-details/${attributeId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
    ...
    const data = await response.json();
    console.log(
      "[FilterAPI] Sub-attributes response for",
      attributeId,
      ":",
      data,
    );
    ...
    console.error(
      "[FilterAPI] Error fetching sub-attributes for",
      attributeId,
      ":",
      error,
    );
...
export default function EmployeeAssignment({
    ...
    const loadEmployees = async () => {
      try {
        ...
        if (!token) {
          ...
          setError(
            "JWT token required. Please obtain a JWT token and visit /id/{your-jwt-token} to authenticate.",
          );
          setLoading(false);
          return;
        }
    ...
    if (showFilterDialog) {
      const loadFilterData = async () => {
        try {
          ...
          console.log("[FilterAPI] Loaded filter data:", {
            workerTypesData,
            attributesData,
          });
        } catch (error) {
  ...
  if (employees.length > 0 && filteredEmployees.length === 0) {
    console.log(
      "[EmployeeAssignment] FILTER DEBUG - All employees filtered out!",
    );
  ...
  const handleAttributeSelection = async (
    conditionId: string,
    attributeId: string,
    attributeName: string,
  ) => {
                        ...
                        onValueChange={(value) => {
                          ...
                          if (condition.type === "workerType") {
                            ...
                            const subAttributeList =
                              subAttributes[condition.attributeId] || [];
                            const subAttribute = subAttributeList.find(
                              (sub) => sub.attribute_sub_id === value,
                            );
                            if (
                              subAttribute &&
                      ...
                      >
                        <SelectTrigger className="flex-1">
                          <SelectValue
                            placeholder={
                              ...
                              condition.selectedLabels.length > 0
                        ...
                        <SelectContent>
                          {condition.type === "workerType"
                            ...
                            : condition.attributeId &&
                                subAttributes[condition.attributeId]
                              ? subAttributes[condition.attributeId].map(
                                  (subAttr) => (
                                    <SelectItem
                                      key={subAttr.attribute_sub_id}
                                      value={subAttr.attribute_sub_id}
                                    >
            ...
            <tbody>
              {currentEmployees.map((employee: Employee) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm text-gray-600">
                    {employee.department || employee.workerType}
                  </td>
                </tr>
              ))}
            </tbody>

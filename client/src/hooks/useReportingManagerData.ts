...
export function useReportingManagerData() {
  ...
  const fetchReportees = async (userId: string) => {
    ...
    try {
      ...
      console.log("[ReportingManager] Making API call to:", `https://apiv1.resolvepay.in/organization/reporting-manager/${userId}/reportees`);
      console.log("[ReportingManager] Using JWT token:", jwtToken?.substring(0, 50) + "...");
      
      const response = await fetch(`https://apiv1.resolvepay.in/organization/reporting-manager/${userId}/reportees`, {
        method: 'GET',
        headers: {

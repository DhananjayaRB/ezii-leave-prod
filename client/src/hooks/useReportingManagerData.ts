...
export function useReportingManagerData() {
  ...
  console.log(
    "[ReportingManager] Hook called - isAuthenticated:",
    isAuthenticated,
    "user:",
    !!user,
  );
  ...
  const fetchReportees = async (userId: string) => {
    ...
    try {
      ...
      if (!jwtToken) {
        console.log(
          "[ReportingManager] No JWT token found in localStorage, showing all employees",
        );
        console.log(
          "[ReportingManager] Available localStorage keys:",
      ...
      if (!response.ok) {
        console.log(
          "[ReportingManager] API call failed with status:",
          response.status,
        );
      ...
      if (
        ...
        console.log(
          `[ReportingManager] User is ${isReportingManager ? "a reporting manager" : "not a reporting manager"} with ${reportees.length} reportees`,
        );
      } else {
        setData({
          reportees: [],
  ...
  useEffect(() => {
    console.log(
      "[ReportingManager] Effect triggered - isAuthenticated:",
      isAuthenticated,
      "user:",
      !!user,
    );
    if (isAuthenticated && user) {
      ...
      console.log(
        "[ReportingManager] User authenticated, calling reporting manager API for user_id:",
        userId,
      );
      console.log(
        "[ReportingManager] localStorage contents:",
        Object.keys(localStorage),
      );
      ...
      console.log(
        "[ReportingManager] Not calling API - isAuthenticated:",
        isAuthenticated,
        "user:",
        !!user,
      );
  ...
  useEffect(() => {
    ...
    console.log(
      "[ReportingManager] Force calling API immediately for testing with user_id:",
      userId,
    );
    console.log(
      "[ReportingManager] Current localStorage keys:",
      Object.keys(localStorage),
    );
    fetchReportees(userId);
  }, []);

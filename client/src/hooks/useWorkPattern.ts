  ...
  useEffect(() => {
    const fetchWorkPatternAndHolidays = async () => {
      try {
        ...
        // Fetch work pattern
        const workPatternResponse = await fetch(`https://apiv1.resolvepay.in/organization/get-work-patterns`, {
          method: 'GET',
          headers: {
        ...
        if (workPatternResponse.ok) {
          ...
          if (workPatternData.data && workPatternData.data.length > 0) {
            ...
            const holidaysResponse = await fetch('https://apiv1.resolvepay.in/organization/holidays', {
              method: 'GET',
              headers: {
                'Authorization': `Bearer ${jwtToken}`,
                'Content-Type': 'application/json'

...
export function useWorkPattern() {
  ...
  useEffect(() => {
    const fetchWorkPatternAndHolidays = async () => {
      try {
        ...
        if (!jwtToken) {
          console.log(
            "⚠️  [WorkPattern] No JWT token found, cannot fetch work pattern",
          );
          setIsLoading(false);
          return;
        }
        ...
        console.log(
          "[WorkPattern] Fetching work patterns for user:",
          currentUserId,
        );

        // Fetch work pattern
        const workPatternResponse = await fetch(
          `https://apiv1.resolvepay.in/organization/get-work-patterns`,
          {
            method: "GET",
            headers: {
        ...
        console.log(
          "[WorkPattern] Work pattern API Response status:",
          workPatternResponse.status,
        );
        ...
        if (workPatternResponse.ok) {
          ...
          console.log(
            "[WorkPattern] Full API response structure:",
            JSON.stringify(workPatternData, null, 2),
          );

          if (workPatternData.data && workPatternData.data.length > 0) {
            // Find the work pattern that includes the current user
            const userPattern =
              workPatternData.data.find(
                (pattern: any) =>
                  pattern.employees &&
                  pattern.employees.includes(parseInt(currentUserId)),
            ...
            console.log("[WorkPattern] Work pattern structure details:", {
              ...
              userIdInPattern: userPattern.employees?.includes(
            ...
            console.log(
              "[WorkPattern] External holidays API response status:",
              holidaysResponse.status,
            );

            if (holidaysResponse.ok) {
              const holidaysData = await holidaysResponse.json();
              console.log(
                "[WorkPattern] External holidays data:",
                holidaysData,
              );
              ...
              if (
                userWorkPattern.selectedHolidays &&
                userWorkPattern.selectedHolidays.length > 0
              ) {
                ...
                console.log(
                  "[WorkPattern] Filtered holidays matching work pattern:",
                  filteredHolidays,
                );
                setHolidays(filteredHolidays);
              } else {
                console.log(
                  "[WorkPattern] No selected holidays in work pattern",
                );
                setHolidays([]);
              }
            } else {
              ...
              console.log(
                "[WorkPattern] Failed to fetch external holidays:",
                holidaysResponse.status,
                errorText,
              );
              setHolidays([]);
          ...
          console.log(
            "[WorkPattern] Work pattern API Error:",
            workPatternResponse.status,
            errorText,
          );
  ...
  const isWorkingDay = (date: Date): boolean => {
    ...
    console.log(
      `[WorkPattern] isWorkingDay check for ${date.toISOString().split("T")[0]} (${dayName}):`,
      {
        dayOfWeek,
        dayName,
    ...
    if (workPattern?.nonWorkingDays && workPattern.nonWorkingDays.length > 0) {
      ...
      const isNonWorking = workPattern.nonWorkingDays.some((pattern) => {
        ...
        const patternMatch = pattern.days.some((patternDay) => {
          ...
          console.log(
            `[WorkPattern] Pattern day "${patternDay}" vs "${dayName}": ${matches}`,
          );
          return matches;
        });
        console.log(
          `[WorkPattern] Pattern result for ${dayName}:`,
          patternMatch,
        );
        return patternMatch;
      });
      ...
      console.log(
        `[WorkPattern] Final isNonWorking result for ${dayName}:`,
        isNonWorking,
      );
      return !isNonWorking;
    }
    ...
    console.log(
      `[WorkPattern] No work pattern, using weekend fallback for ${dayName}: isWeekend=${isWeekend}`,
    );
    return !isWeekend;
  };

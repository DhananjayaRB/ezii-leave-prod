                ...
                <div className="text-lg sm:text-2xl font-bold text-blue-600">
                  {(() => {
                    ...
                    availableLeaveVariants.forEach((variant: any) => {
                      ...
                      if (variant.leaveTypeName === 'Earned Leave') {
                        console.log('🔍 [EARNED LEAVE] FINAL DEBUG:', {
                          variantPaidDaysInYear: variant.paidDaysInYear,
                          balanceTotalEntitlement: balance?.totalEntitlement,
                          totalEntitlementInDays,
                          openingBalance,
                          isAfterEarning,
                          grantFrequency: variant.grantFrequency,
                          calculation: `(${variant.paidDaysInYear}/12)*8=${(variant.paidDaysInYear/12)*8}`,
                          shouldBe12: true
                        });
                      }
                      ...
                      if (isAfterEarning) {
                        ...
                        // FIXED: Always use variant.paidDaysInYear for correct calculation
                        const annualEntitlement = variant.paidDaysInYear || 0;
                        ...
                        if (variant.grantFrequency === 'per_year') {
                          ...
                          // FIXED: Monthly grants - pro-rated amount based on elapsed months from joining date
                          const currentMonth = new Date().getMonth(); // August = 7 (0-based)
                          const elapsedMonths = currentMonth + 1; // Jan-Aug = 8 months
                          eligibility = (annualEntitlement / 12) * elapsedMonths;
                          
                          if (variant.leaveTypeName === 'Earned Leave') {
                            console.log('🚨 [EARNED LEAVE FIX] Current values:', {
                              isAfterEarning,
                              ...
                              elapsedMonths,
                              eligibility,
                              expectedForInAdvance: (18 / 12) * 8,
                              actualCalculation: `(${annualEntitlement}/12)*${elapsedMonths}=${eligibility}`
                            });
                          }
                        }
                      }
